...
# Add time‑series and deep learning models for prediction
if STATSMODELS_AVAILABLE:
    REGRESSION_MODELS.append("ARIMA (time series)")
if TF_AVAILABLE:
    REGRESSION_MODELS.append("LSTM (deep learning)")
...
def get_anomaly_scores(model_name, X_train, X_test, params):
    ...
    # ----- VAR Residual (time‑series) -----
    elif model_name == "VAR Residual (time series)":
        train_data = X_train.values
        test_data = X_test.values
        # Fit VAR on original data (we assume stationarity or let user difference)
        ...
        native_threshold = np.percentile(train_scores, params["contamination"] * 100)
        model = fitted  # store fitted model
...
def get_regressor(model_name, params):
    """For classic sklearn regressors."""
...
with tab1:
    ...
    if st.session_state.det_result is not None:
        ...
        # ----- ADDED: Info expander explaining each method in simple terms -----
        with st.expander("ℹ️ What do these threshold methods mean?"):
            st.markdown("""
            - **Auto (model's native threshold)** – Uses the threshold that the model itself learned during training (for example, Isolation Forest has an internal `offset`).  
              ✅ Best when you trust the model's built‑in sensitivity to anomalies.

            - **Percentile (worst X%)** – Flags the lowest X% of anomaly scores.  
              💡 For example, if you set **5%**, the 5% of points with the *lowest* scores (most anomalous) are flagged.  
              ✅ Simple and intuitive – you directly control how many points are flagged.

            - **Mean - K*Std** – Flags points whose score is **K standard deviations** below the average score.  
              📊 This works well if your scores look roughly like a bell‑shaped (normal) distribution.  
              ✅ Good for detecting extreme outliers.

            - **Manual value** – You draw a hard line. Any point with a score **below** this value is flagged.  
              🎯 Use this when you have domain knowledge and know exactly which score is the danger limit.
            """)
...
with tab2:
    if st.session_state.det_result is None or "anomaly" not in st.session_state.det_result.columns:
        ...
        # ----- NEW: LSTM (deep learning) -----
        elif reg_name == "LSTM (deep learning)":
            with r1:
                rparams["lookback"] = st.slider("Lookback (time steps)", 2, 30, 10, key="lstm_pred_look")
            with r2:
                rparams["units"] = st.slider("LSTM units", 8, 128, 32, key="lstm_pred_units")
            with r3:
                rparams["epochs"] = st.slider("Epochs", 10, 100, 50, key="lstm_pred_epochs")
            with r1:
                rparams["batch_size"] = st.slider("Batch size", 8, 128, 32, key="lstm_pred_batch")
            if not TF_AVAILABLE:
                st.warning("TensorFlow not installed. Install with `pip install tensorflow`.")
        ...
        if run_pred:
            ...
            with st.spinner("Training prediction model..."):
                ...
                if reg_name in ["Random Forest", "Extra Trees", "XGBoost", "Gradient Boosting",
                    ...
                    # ARIMA uses only the target (univariate)
