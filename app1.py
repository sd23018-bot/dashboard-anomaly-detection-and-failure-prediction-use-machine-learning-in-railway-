...
def get_anomaly_scores(model_name, X_train, X_test, params):
    ...
    # ----- AUTOENCODER (Deep Learning) -----
        ...
        # Use fixed defaults (no user parameters)
        seq_len = 10
        units = 32
        epochs = 50
        batch_size = 32
        contamination = 0.04  # default for native threshold
        ...
        native_threshold = np.percentile(train_scores, contamination * 100)
...
def get_regressor(model_name, params):
    ...
    if model_name == "LSTM (Deep Learning)":
        ...
        return ("LSTM", {"lookback": 10, "units": 32, "epochs": 50, "batch_size": 32})
...
with tab1:
    ...
    params = {}
    # Only show hyperparameter controls for classic models
    if model_name == "Isolation Forest":
        ...
        st.info("🔧 Using default parameters: sequence length = 10, LSTM units = 32, epochs = 50, batch size = 32")
        if not TF_AVAILABLE:
            st.warning("⚠️ TensorFlow is not installed. Please run: `pip install tensorflow`")
        # No sliders; params will be set internally in get_anomaly_scores
    ...
    if run_detect:
        with st.spinner("Fitting model..."):
            try:
                # For Autoencoder, params is empty; get_anomaly_scores will use defaults
...
with tab2:
    if st.session_state.det_result is None or "anomaly" not in st.session_state.det_result.columns:
        ...
        rparams = {}
        # Show hyperparameter controls only for classic models
        if reg_name == "Random Forest":
            ...
            st.info("🔧 Using default parameters: p=1, d=1, q=1")
            if not STATSMODELS_AVAILABLE:
                st.warning("⚠️ statsmodels is not installed. Please run: `pip install statsmodels`")
            # No sliders
        elif reg_name == "LSTM (Deep Learning)":
            st.info("🔧 Using default parameters: lookback=10, units=32, epochs=50, batch_size=32")
            if not TF_AVAILABLE:
                st.warning("⚠️ TensorFlow is not installed. Please run: `pip install tensorflow`")
            # No sliders
        ...
        if run_pred:
            ...
            with st.spinner("Training prediction model..."):
                try:
                    # Get the regressor placeholder (for classic models, returns the model; for ARIMA/LSTM, returns a tuple)
                ...
                # ----- ARIMA -----
                    ...
                    # reg_placeholder is ("ARIMA", params) with defaults
                    _, arima_params = reg_placeholder
                    model = ARIMA(y_train, order=(arima_params["p"], arima_params["d"], arima_params["q"]))
                    fitted = model.fit()
