...
from scipy.stats import norm   # for ASOI calculation
...
def compute_asoi(scores, percentile=5):
    """
    Compute Anomaly Separation and Overlap Index (ASOI).
    Treats the lowest 'percentile'% of scores as anomalies, the rest as normal.
    Returns a value between 0 and 1 (higher = better model).
    """
    scores = np.asarray(scores).flatten()
    if len(scores) < 10:
        return 0.0  # not enough data

    # Split into anomaly and normal groups
    threshold = np.percentile(scores, percentile)
    anom_scores = scores[scores <= threshold]
    norm_scores = scores[scores > threshold]

    if len(anom_scores) < 2 or len(norm_scores) < 2:
        return 0.0  # cannot compute robustly
    ...
    # 1. Separation: Cohen's d (effect size)
    pooled_std = np.sqrt((sigma_a**2 + sigma_n**2) / 2)
    if pooled_std == 0:
        d = 0.0
    else:
        d = (mu_n - mu_a) / pooled_std  # positive because anomalies have lower scores

    # 2. Overlap coefficient for two normal distributions (exact formula)
    # Solve for x0 where f1(x0) = f2(x0)
    # We assume mu_a < mu_n (since anomaly scores are lower)
    if mu_a > mu_n:
        # Swap to ensure mu1 < mu2
        mu_a, mu_n = mu_n, mu_a
    ...
    # Avoid numerical issues if a is tiny (equal variances)
    if abs(a) < 1e-10:
        # Equal variances: x0 is the midpoint
        x0 = (mu_a + mu_n) / 2
    else:
        disc = b**2 - 4*a*c
        if disc < 0:
        ...
        # Choose the root that lies between mu_a and mu_n
        x1 = (-b + sqrt_disc) / (2*a)
        x2 = (-b - sqrt_disc) / (2*a)
        if mu_a <= x1 <= mu_n:
            x0 = x1
            ...
            # fallback to the one closer to the midpoint
            mid = (mu_a + mu_n) / 2
            if abs(x1 - mid) < abs(x2 - mid):
                x0 = x1
            else:
                x0 = x2

    # Compute overlap coefficient
    # OVL = Φ((x0 - mu_a)/sigma_a) + 1 - Φ((x0 - mu_n)/sigma_n)
    ovl = norm.cdf((x0 - mu_a) / sigma_a) + (1 - norm.cdf((x0 - mu_n) / sigma_n))
    # Clamp to [0,1]
    ovl = min(max(ovl, 0.0), 1.0)

    # 3. ASOI = d * (1 - ovl)   (both in [0,1] range)
    # Normalize d to [0,1] using a sigmoid-like transformation: 1 - exp(-d)
...
# --------------------------------------------------------------------------
# SIDEBAR — ANOMALY DETECTION CONTROLS (updated with ASOI)
# --------------------------------------------------------------------------
...
with col2:
    if st.button("🚀 Auto Find Best"):
        ...
        with st.spinner("Running all models..."):
            for m in ANOMALY_MODELS:
                try:
                    ...
                    # Compute ASOI
                    asoi_val = compute_asoi(all_scores, percentile=5)
                    asoi_dict[m] = asoi_val
                    # Also compute raw gap for reference (optional)
                    thresh = np.percentile(all_scores, 5)
                    low_avg = np.mean(all_scores[all_scores <= thresh])
                    high_avg = np.mean(all_scores[all_scores > thresh])
                    raw_gap_dict[m] = high_avg - low_avg
                except Exception as e:
                    asoi_dict[m] = -np.inf
                    raw_gap_dict[m] = -np.inf
        ...
        if not valid_asoi:
            ...
            # Show comparison table (both raw gap and ASOI)
            comp_df = pd.DataFrame({
                'Raw Gap': raw_gap_dict,
                'ASOI': asoi_dict
            }).sort_values('ASOI', ascending=False)
            st.sidebar.dataframe(comp_df)

# Threshold method (only show if det_result exists)
...
if st.session_state.det_result is None or "anomaly" not in st.session_state.det_result.columns:
    ...
    if (st.session_state.get("forecast_df") is not None and 
        ...
        # Metric cards removed – no m1, m2, m3 columns
    else:
...
# (Data preview & correlation expander removed per user request)
