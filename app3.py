...
for key in ["df_raw", "df", "feature_columns", "train", "test", "scaler",
            ...
            "forecast_df", "forecast_sensor", "forecast_thresholds"]:
...
with col2:
    if st.button("🚀 Auto Find Best"):
        ...
        with st.spinner("Running all models..."):
            ...
            valid = {k: v for k, v in raw_scores_dict.items() if v != -np.inf}
            if valid:
                min_val = min(valid.values())
                max_val = max(valid.values())
                norm_scores = {}
                for m, raw in raw_scores_dict.items():
                    if raw == -np.inf:
                        norm_scores[m] = -np.inf
                    elif max_val == min_val:
                        norm_scores[m] = 0.0
                    else:
                        norm_scores[m] = (raw - min_val) / (max_val - min_val)
                best = max(norm_scores, key=norm_scores.get)
                st.session_state["best_anomaly_model"] = best
                # Refit best
                _, train_s, test_s, native_thr = fit_anomaly_model(best, train[feature_columns], test[feature_columns])
                full_scores = pd.Series(
                    np.concatenate([train_s, test_s]),
                    index=list(train.index) + list(test.index),
                    name="anomaly_score",
                ...
                result["split"] = ["train"] * len(train_s) + ["test"] * len(test_s)
                st.session_state.det_result = result
                st.session_state.det_model_name = best
                st.session_state.native_threshold = native_thr
                st.sidebar.success(f"✅ Best: {best}")
                # Removed the quality score table
            else:
...
if st.session_state.det_result is not None:
    ...
    # Apply threshold automatically when method or parameters change
    try:
...
if st.session_state.det_result is not None and "anomaly" in st.session_state.det_result.columns:
    ...
    if st.sidebar.button("🚀 Find Best Prediction Model"):
        ...
        with st.spinner("Running prediction models..."):
            ...
            if best_reg is not None:
                ...
                st.sidebar.success(f"Best: {best_reg} (R²={r2:.4f})")
                # Removed the R² comparison table display
            else:
                st.sidebar.error("All models failed.")
else:
...
if st.session_state.det_result is None or "anomaly" not in st.session_state.det_result.columns:
    ...
    if st.button("🚀 Generate Forecast"):
        ...
        df_forecast = build_linear_forecast(det[sensor_col], horizon, 100)  # fixed trend window
        df_forecast["anomaly"] = (
    ...
    if (st.session_state.get("forecast_df") is not None and 
        st.session_state.get("forecast_sensor") == sensor_col and
...
if st.session_state.det_result is not None and "anomaly" in st.session_state.det_result.columns:
    ...
    with col2:
        exp_window = st.slider("Window size", 2, 20, 5, key="exp_window_main")
    with col3:
        default_spike = float(round(det[exp_sensor].std() * 1.5, 2)) or 2.5
        exp_spike = st.number_input("Spike threshold (Δ)", value=default_spike, step=0.1, key="exp_spike_main")
    with col4:
        default_slope = float(round(det[exp_sensor].std() * 0.3, 2)) or 0.3
        exp_slope = st.number_input("Slope threshold", value=default_slope, step=0.05, key="exp_slope_main")
