...
if st.session_state.det_result is not None:
    ...
    # Create a row with selectbox and a help button
    thr_col1, thr_col2 = st.sidebar.columns([4, 1])
    with thr_col1:
    ...
    with thr_col2:
        # Help button with "?" icon that opens a popover
        if st.button("? ⓘ", key="thr_help_btn"):
            st.popover(
                "**Threshold Method Explanations**\n\n"
...
if st.session_state.det_result is not None and "anomaly" in st.session_state.det_result.columns:
    ...
    # Two columns: Fit Manual and Auto Find Best
        ...
        if st.button("Fit Manual Prediction"):
            with st.spinner("Training selected model..."):
                try:
                    ...
                    # Compute metrics for internal use (not displayed)
