...
with tab1:
    ...
    # If auto-run is clicked, run all models and select best
    if auto_run:
        scores_dict = {}
        models_dict = {}
        with st.spinner("Running all anomaly models... This may take a moment."):
            for m in ANOMALY_MODELS:
                try:
                    _, train_s, test_s, _ = fit_anomaly_model(m, train[feature_columns], test[feature_columns])
                    quality = evaluate_anomaly_model(train_s, test_s)
                    scores_dict[m] = quality
                    models_dict[m] = (train_s, test_s)
                except Exception as e:
                    scores_dict[m] = -np.inf  # skip if fails
                    st.warning(f"⚠️ {m} failed: {e}")
            # Select best
            best_model = max(scores_dict, key=scores_dict.get)
            st.session_state["best_anomaly_model"] = best_model
            st.success(f"✅ Best model selected: **{best_model}** (score: {scores_dict[best_model]:.2f})")
            # Fit best model again (to store scores)
            ...
            comp_df = pd.DataFrame.from_dict(scores_dict, orient='index', columns=['Quality Score']).sort_values('Quality Score', ascending=False)
            st.dataframe(comp_df, use_container_width=True)

    # Manual fit button (uses selected model)
    if st.button("Fit selected model", type="secondary"):
        with st.spinner("Fitting model..."):
            try:
                _, train_s, test_s, native_thr = fit_anomaly_model(model_name, train[feature_columns], test[feature_columns])
